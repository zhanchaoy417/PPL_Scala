...
class CanvasTests extends FunSuite {
    ...
    test("Test Place Right"){
        ...
        val shiftedPolygon = new Polygon(List((-.5,1), (0.5,1), (0,0)))
        val lst = myc.getListOfObjects
        assert(lst.contains(cir1))
    ...
    test("Test Place Top"){
        ...
        val shiftedPolygon = new Polygon(List((1,2.5), (2,2.5), (1.5,1.5)))
        val lst = myc.getListOfObjects
        assert(lst.contains(cir1))
        assert(lst.contains(shiftedPolygon))
    }
    ...
    test("Test Rotate 0") {
        val cir1 = new MyCircle ((0,0), 0.5)
        val myc = new MyCanvas(List(cir1))
        val myc2 = myc.rotate(3.1415/2)
        println(myc2)
        val lst = myc2.listOfObjects
        // Rotation about center of bounding box does not change cir1
        assert(lst.contains(cir1))
    }

    test("Test Rotate 1"){
        val cir1 = new MyCircle((1,1), 0.5)
        val myc = new MyCanvas(List(cir1))
        val myc2 = myc.rotate(3.1415/2)
        println(myc2)
        val lst = myc2.listOfObjects
    ...
    test("Test Rotate 2"){
        val cir1 = new MyCircle((1,1), 0.5)
        val cir2 = new MyCircle((-1,-1),0.5)
        val myc = new MyCanvas(List(cir1, cir2))
        val myc2 = myc.rotate(3.1415/4)
        println(myc2)
        val lst = myc2.listOfObjects
        // Rotation about center of bounding box does not change cir1
        assert(lst.length == 2)
        val reqdCircle1 = new MyCircle((0.0,1.414213561993702),0.5)
        assert(lst.exists {
            case c@MyCircle(_,_) => {
                compareCircles(c, reqdCircle1) <= 1E-03
            }
            case _ => false
        })
        val reqdCircle2 = new MyCircle((0.0,-1.414213561993702),0.5)
        assert(lst.exists {
            case c@MyCircle(_,_) => {
                compareCircles(c, reqdCircle2) <= 1E-03
            }
            case _ => false
        })
    }
    ...
    test("Test Rotate 3"){
        ...
        //
        val reqdCircle = new MyCircle((-0.9575380626537373,0.34152499802012515),0.5)
        assert(lst.exists {
                case c@MyCircle(_,_) => {
        ...
        val reqdPoly = new Polygon(List((0.7745281869735894,1.3414982511133902), (1.6405613117872528,1.8414848776600228), (1.7075313759270536,0.7254584395730431)))//new Polygon(List((0.36604,1.3660), (1.2320,1.8660), (1.2990,0.7499)))
        assert(lst.exists {
